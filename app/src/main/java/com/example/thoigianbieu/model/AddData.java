...
public class AddData {
    public static void AddMonHoc(Context context){
        ...
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("thcsthaysau", "Thực hành chuyên sâu", "Thầy Sáu", "1B12", buoiHoc, calendar1,calendar2));


        buoiHoc.clear();
        buoiHoc.add("2C");
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("kpdldpt", "Khai phá dữ liệu đa phương tiện", "Thầy Sáu", "1B12", buoiHoc, calendar1,calendar2));

        buoiHoc.clear();
        buoiHoc.add("3S");
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("ttltvud", "Truyền thông lí thuyết và ứng dụng", "Cô Điệp", "1A001", buoiHoc, calendar1,calendar2));

        buoiHoc.clear();
        buoiHoc.add("4C");
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("ltkxha", "Lập trình kĩ xảo hình ảnh", "Thầy Hóa", "1B12", buoiHoc, calendar1,calendar2));

        buoiHoc.clear();
        buoiHoc.add("5S");
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("thcsthayhoa", "Thực hành chuyên sâu", "Thầy Hóa", "1B12", buoiHoc, calendar1,calendar2));

        buoiHoc.clear();
        buoiHoc.add("6S");
        MonHocDatabase.getInstance(context).monHocDAO().insertMonHoc(
                new MonHoc("ptdvgtgttmvt", "Phát triển dịch vụ giá trị gia tăng trên mạng viễn thông", "Thầy Hào", "1B12", buoiHoc, calendar1,calendar2));
    }
